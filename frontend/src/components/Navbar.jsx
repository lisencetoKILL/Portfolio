...
const Navbar = ({ visible = true }) => {
  ...
  return (
    <nav className="fixed right-6 md:right-12 top-1/2 -translate-y-1/2 z-50 flex-col items-center gap-6 hidden md:flex">
      {/* Vertical track line */}
      ...
      {navItems.map((item) => {
        ...
        return (
          ...
          >
              ...
              <span
                className={`block text-[10px] font-mono tracking-[0.2em] uppercase transition-all duration-500 transform
                  ${isActive
                    ...
                    : 'text-slate-500 opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0'
                  }`}
              >
                {item.label}
              </span>
            ...
            >
              {!isActive && (
                <div className="w-[1px] h-[1px] bg-slate-400 rounded-full" />
              )}
